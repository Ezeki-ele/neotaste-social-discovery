...
export default function RestaurantScreen({ restaurantId, onBack, onBookDeal }: Props) {
  ...
  const tabLabels = { overview: 'Übersicht', loyalty: 'Loyalty', reviews: 'Reviews', about: 'Über uns' };
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '4px 16px 12px' }}>
          <h1 style={{
            ...
            lineHeight: 1.25, fontFamily: 'Poppins, system-ui', marginBottom: '8px',
          }}>
            {restaurant.name}
          </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: '#0a0a0a', fontFamily: 'Poppins, system-ui' }}>
              ★ {restaurant.rating}
            </span>
        ...
        <div style={{ display: 'flex', gap: '8px', padding: '0 16px 16px' }}>
          {[
            ...
            <button key={btn.label} style={{
              ...
              <span style={{ fontSize: '11px', fontWeight: 500, color: '#0a0a0a', fontFamily: 'Poppins, system-ui' }}>
                {btn.label}
              </span>
            </button>
          ))}
        ...
        <div style={{ padding: '0 16px 16px', display: 'flex', gap: '6px', height: '180px' }}>
          <div style={{
            flex: 2, borderRadius: '12px',
            background: `linear-gradient(135deg, ${restaurant.gradientFrom}, ${restaurant.gradientTo})`,
          }} />
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <div style={{
              flex: 1, borderRadius: '12px',
              background: `linear-gradient(135deg, ${restaurant.gradientTo}, ${restaurant.gradientFrom})`,
            }} />
            <div style={{
              flex: 1, borderRadius: '12px',
              background: `linear-gradient(to bottom, ${restaurant.gradientFrom}88, ${restaurant.gradientTo})`,
            }} />
          </div>
        </div>
          ...
          {tabs.map(t => (
            <button
              ...
              style={{
                ...
                borderBottom: activeTab === t ? '2px solid #11301d' : '2px solid transparent',
                marginBottom: '-1px',
                cursor: 'pointer',
                fontSize: '13px', fontWeight: activeTab === t ? 700 : 500,
                color: activeTab === t ? '#0a0a0a' : '#737373',
        ...
        {activeTab === 'overview' && (
          <div style={{ padding: '20px 16px 32px' }}>
              ...
              {restaurant.deals.map(deal => (
                <div key={deal.id} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '8px', gap: '8px' }}>
                    <span style={{ fontSize: '15px', fontWeight: 700, color: '#08180f', fontFamily: 'Poppins, system-ui', flex: 1 }}>
                      {deal.title}
                    </span>
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                      <span style={{
                        background: 'rgba(8,24,15,0.1)', borderRadius: '6px', padding: '3px 8px',
                        fontSize: '11px', fontWeight: 600, color: '#08180f', fontFamily: 'Poppins, system-ui',
                      }}>
                        {deal.avgPrice}
                      </span>
                      <span style={{
                  ...
                  {/* Description */}
                  <p style={{
                    fontSize: '13px', color: 'rgba(8,24,15,0.65)', lineHeight: 1.5,
                    marginBottom: '10px', fontFamily: 'Poppins, system-ui',
                  }}>
                    {deal.description}
                  </p>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: '#11301d', border: 'none', cursor: 'pointer',
                      fontSize: '14px', fontWeight: 700, color: '#53f293',
                      fontFamily: 'Poppins, system-ui',
                      transition: 'transform 100ms',
                    }}
            ...
            <div style={{ marginTop: '28px' }}>
              ...
              <div style={{
                marginTop: '12px', background: '#f5f5f5', borderRadius: '12px', padding: '16px',
                display: 'flex', alignItems: 'center', gap: '12px',
              }}>
                <div style={{
                  width: 44, height: 44, borderRadius: '10px', background: '#53f293',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  <span style={{ fontSize: '22px' }}>⭐</span>
                </div>
                <div>
                  <p style={{ fontSize: '14px', fontWeight: 700, color: '#0a0a0a', fontFamily: 'Poppins, system-ui' }}>
                    0 von 5 Besuchen
                  </p>
                  <p style={{ fontSize: '12px', color: '#737373', fontFamily: 'Poppins, system-ui' }}>
                    Sammle Stempel beim Einlösen von Deals
                  </p>
                </div>
              </div>
            </div>
        ...
        {activeTab !== 'overview' && (
          <div style={{
            ...
            <span style={{ fontSize: '32px' }}>
              {activeTab === 'loyalty' ? '⭐' : activeTab === 'reviews' ? '💬' : 'ℹ️'}
            </span>
            <p style={{ fontSize: '15px', fontWeight: 700, color: '#0a0a0a', fontFamily: 'Poppins, system-ui' }}>
              {activeTab === 'loyalty' ? 'Loyalty Programm' : activeTab === 'reviews' ? 'Noch keine Reviews' : 'Über das Restaurant'}
            </p>
            <p style={{ fontSize: '13px', color: '#737373', textAlign: 'center', fontFamily: 'Poppins, system-ui' }}>
              {activeTab === 'reviews' ? 'Sei der Erste und hinterlasse eine Bewertung.' : 'Informationen folgen bald.'}
            </p>
          </div>
        )}
      </div>
